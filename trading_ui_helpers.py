...
def safe_pnl(value: Any) -> Optional[float]:
    ...
    Handles None and common conversion errors (TypeError, ValueError).
    """
    if value is None:
        return None
    try:
...
def safe_stat(stats: Dict[str, Any], key: str, default: float = 0.0) -> float:
    ...
    If key is missing or value is not numeric, returns the default value.
    """
...
def fmt_price(value: Any, decimals: int = 4) -> str:
    ...
    Returns an em dash (—) if value is invalid or None.
    """
    if value is None:
        return "—"
    try:
...
def primary_btn(
    ...
    The button uses the primary theme colors and changes to a darker shade on hover.
    """
    btn = tk.Button(
        parent,
        text=text,
...
class AutocompleteCombobox(ttk.Combobox):
    ...
    Usage:
        combo = AutocompleteCombobox(parent)
        combo.set_completion_list(["Option1", "Option2", ...])
    """
...
def get_ccxt_exchanges() -> List[str]:
    """
    Return sorted list of supported exchanges (from ccxt if available).

    The result is cached; to clear the cache call `get_ccxt_exchanges.cache_clear()`.
    """
    ids: set = {"simulator"}
    try:
        import ccxt
        ids.update(x.lower() for x in getattr(ccxt, "exchanges", []) if x)
    except Exception:
        # ccxt not installed or error during import – silently ignore
        pass
    return sorted(ids)