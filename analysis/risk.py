# analysis/risk.py  (v4.2 — Harmonized with MA Trend-Following Strategy)
"""
Multi-Engine Crypto Risk Assessment Engine v4.2
────────────────────────────────────────────────────────────────────────────
CHANGES FROM v4.1
─────────────────
...
FEATURE_NORM_RANGES: Dict[str, Tuple[float, float]] = {
    ...
    # NEW ranges
    "MA20_Dev":      (0.0, 3.0),   # normalized by ATR, typical range 0..3
...
_ONE_SIDED_KEYS: frozenset = frozenset({
    ...
    "MA20_Dev", "MA_Cross",          # one-sided (absolute deviation/separation)
})
...
def _build_engine_specs() -> Dict[str, Tuple[FactorSpec, ...]]:
    ...
    return {
        ...
        "trend": (
            ...
            # NEW MA-based trend risk factors
...
def _derive_features(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # NEW: Derive MA20_Dev and MA_Cross if not present
    ...
    if ma20 is not None and price is not None and ma20 > 0:
        ...
        if atr_pct is not None and atr_pct > 0:
            derived["MA20_Dev"] = abs(price - ma20) / ma20 * 100.0 / atr_pct
        else:
            derived["MA20_Dev"] = abs(price - ma20) / ma20 * 100.0  # fallback
...
# ════════════════════════════════════════════════════════════
# v4.2: RISK CACHE (Thread-Safe) — unchanged from v4.1
# ════════════════════════════════════════════════════════════

class RiskCache:
    def __init__(self, ttl_sec: Optional[int] = None, max_size: Optional[int] = None):
        self._ttl = ttl_sec or int(CONFIG.get("cache_ttl_sec", 300))
        self._max = max_size or int(CONFIG.get("cache_max_size", 10_000))
    ...
    def get_or_compute(self, row: Dict[str, Any], compute_fn: Callable[[Dict[str, Any]], Dict[str, Any]]) -> Dict[str, Any]:
...
# ════════════════════════════════════════════════════════════
# EXPLAINERS & COMPARATORS — unchanged
# ════════════════════════════════════════════════════════════
