# analysis/indicators_integration.py  (v4.4 — MA Trend Integration)
"""
Data Enrichment & Decision Engine v4.4
════════════════════════════════════════════════════════════════════════════
CHANGES FROM v4.3
──────────────────
- ADDED MA20/MA50 columns in enrich_market_data output
  (rolled from price column, if not already present)
...
class ConfidenceBreakdown:
    ...
    def to_dict(self) -> Dict[str, float]:
        precision = Config.CONFIDENCE_PRECISION
        return {
            "base":         round(self.base, precision),
            "signal_part":  round(self.signal_part, precision),
            "quality_part": round(self.quality_part, precision),
            "risk_penalty": round(self.risk_penalty, precision),
            "final":        round(self.final, precision),
        }
...
def validate_dataframe(
    ...
    if len(df) < min_rows:
        raise ValidationError(f"{prefix}DataFrame has {len(df)} rows, minimum {min_rows} required")
    if required_columns:
...
def required_warmup(cfg_hash: Optional[int] = None) -> int:
    cfg = INDICATOR_CONFIG
    def safe_int(key: str, default: int) -> int:
        try:
            return max(1, int(cfg.get(key, default)))
        except (TypeError, ValueError):
            logger.warning("Invalid config value for %s, using default %d", key, default)
            return default
    ...
    requirements = [
        ...
        # MA50 نیاز به 50 شمع دارد
        50,
    ]
    warmup = max(requirements)
    logger.debug(
        "Calculated warmup: %d bars (EMA_slow=%d)",
...
def build_decision_reasons(
    ...
    if action == Action.HOLD.value:
        ...
        if level == RiskLevel.EXTREME.value:
            ...
            min_q = SIGNAL_CONFIG.get(
                "min_quality_for_long",
                Config.MIN_QUALITY_DEFAULT
            )
            reasons.append(
...
def get_trading_decision(
    ...
    risk_level = str(
        risk_info.get("level") or 
        risk_info.get("risk_level") or 
        risk_info.get("Risk_Level") or 
        risk_info.get("risk") or 
        RiskLevel.MEDIUM.value
    )
    risk_score_100 = float(
        risk_info.get("score") or 
        risk_info.get("Risk_Score") or 
        50.0
    )
