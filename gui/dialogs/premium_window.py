...
"""
PremiumWindow v2.4 — Activation Code Edition
...
class PremiumWindow(BaseDialog):
    ...
    def _verify_worker(self, tx_hash: str, months: int, device_id: Optional[str]) -> None:
        try:
            ok, error_msg = verify_usdt_transaction(tx_hash=tx_hash, months=months, device_id=device_id)
            if not ok:
