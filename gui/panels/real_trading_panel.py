...
Version: 7.4.6 — Start/Stop actually control live auto entries; flush UI queue.
"""
...
class RealTradingPanel(tk.Frame):
        ...
        try:
            ...
            if self._is_live_exchange() and self.tracker.position_size_mode == "fixed":
                self.tracker.fixed_position_quote = max(
                    self.tracker.fixed_position_quote, self.tracker.min_notional_quote
                )
            if not self._is_live_exchange():
                self.tracker.account_balance = float(getattr(self.config, "account_balance", 1000.0))
                self.tracker.initial_balance = float(getattr(self.config, "account_balance", 1000.0))
                self.tracker.cash = self.tracker.account_balance
                self.tracker.peak_equity = self.tracker.account_balance

            self.tracker.min_volume_24h = float(getattr(self.config, "min_volume_24h", 100000.0))
            self.tracker.min_market_cap = float(getattr(self.config, "min_market_cap", 0.0))
            self.tracker.use_risk_filter = bool(getattr(self.config, "use_risk_filter", False))
            self.tracker.blocked_risk_levels = list(
                getattr(self.config, "blocked_risk_levels", ["High", "Extreme"])
            )
            self.tracker.min_quality = float(getattr(self.config, "min_quality", 0.0))
            self.tracker.max_new_entries_per_cycle = int(
                getattr(self.config, "max_new_entries_per_cycle", 3)
            )
            self.tracker.cooldown_after_loss_min = int(
                getattr(self.config, "cooldown_after_loss_min", 15)
            )
            self.tracker.cooldown_after_win_min = int(
                getattr(self.config, "cooldown_after_win_min", 15)
            )
            self.tracker.quote_currency = str(
                getattr(self.config, "quote_currency", None)
                or getattr(self.bot, "quote_currency", "IRT")
                or "IRT"
            ).upper()
            self.tracker._save_state()
            logger.debug("Tracker parameters updated.")
        except Exception as e:
    ...
    def _is_symbol_unsupported(self, symbol: str) -> bool:
        ...
        logger.debug("Symbol %s is not supported on %s (cached).", base, self.bot.exchange_name)
        return True
    ...
    def _toggle_auto_trade(self) -> None:
        ...
        if self._is_live_exchange():
            ...
            if wanted:
                self._auto_status_lbl.config(text="✅ Global Lead Active", fg=T.SUCCESS_DARK)
                self._set_running_ui(True)
                self.log("Auto entries ON (Global Lead → Nobitex). CMC pump queue stays off.", "success")
            else:
                self._auto_status_lbl.config(text="(Paused)", fg=T.TEXT_MUTED)
...
class BotSettingsWindow(BaseDialog):
    _RISK_FIELDS: List[Tuple[str, str, float]] = [
        ...
        ("Trailing Distance %", "trailing_distance_pct", 2.0),
        ("Max daily loss %", "max_drawdown_percent", 20.0),
    ]
    ...
    def _build_exchange_section(self, parent) -> None:
        ...
        self._exchange_var = tk.StringVar(value=self.config.exchange or "")
        self._exchange_cb = AutocompleteCombobox(ef, textvariable=self._exchange_var, width=22)
    ...
    def _reset_to_defaults(self) -> None:
        ...
        self._risk_vars["stop_loss_pct"].set(self.DEFAULT_SETTINGS["stop_loss_pct"])
